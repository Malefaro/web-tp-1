...
class QuestionForm(forms.Form):
    ...
    def check_tag(self, tag):
        if ((' ' in tag) or ('\n' in tag) or('\t' in tag)) :
            raise forms.ValidationError('Tags contain spaces.Write tags using ","')
    ...
    def save(self, user, id):
        data = self.cleaned_data
        if (id <= 0):
            q = Question.objects.create(title=data.get('title'), text=data.get('text'), author=user, is_active=True)
        else:
        ...
        for tag_text in self._tag_list:
            if (tag_text is not None and tag_text != ''):
                tag = Tag.objects.get_or_create(title=tag_text)
                q.tags.add(tag[0])
        q.save()
        return q
